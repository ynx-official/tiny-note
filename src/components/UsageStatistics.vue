...
async function loadStats() {
  ...
  try {
    ...
    error.value = cause?.message || '用量统计读取失败'
  } finally {
    loading.value = false
  }
}
...
const sourceLabel = source => ({ chat: '对话', note_ai: '笔记 AI', fim: '智能续写', agent: '助手', rag: '知识库' }[source] || source || '其他')
...
<template>
  <section class="assistant-panel usage-panel">
    ...
    <div class="assistant-panel-body usage-body">
      ...
      <template v-else>
        <div class="usage-summary-grid"><div class="usage-summary-card primary"><small>{{ t('totalTokens') }}</small><strong>{{ formatNumber(stats.summary.totalTokens) }}</strong></div><div class="usage-summary-card"><small>{{ t('promptTokens') }}</small><strong>{{ formatNumber(stats.summary.totalPrompt) }}</strong></div><div class="usage-summary-card"><small>{{ t('completionTokens') }}</small><strong>{{ formatNumber(stats.summary.totalCompletion) }}</strong></div><div class="usage-summary-card"><small>{{ t('reasoningTokens') }}</small><strong>{{ formatNumber(stats.summary.totalReasoning) }}</strong></div><div class="usage-summary-card"><small>{{ t('requests') }}</small><strong>{{ formatNumber(stats.summary.totalRequests) }}</strong></div></div>
        ...
        <section v-if="stats.byModel?.length" class="usage-section"><h3>{{ t('byModel') }}</h3><div class="usage-list"><div v-for="(item, index) in stats.byModel" :key="item.key" class="usage-list-row"><div class="usage-list-name"><i :style="{ background: colors[index % colors.length] }"></i>{{ item.label }}</div><div class="usage-list-value"><strong>{{ formatNumber(item.totalTokens) }}</strong><small>{{ formatNumber(item.requests) }} {{ t('requests') }}</small></div><div class="usage-list-bar"><span :style="{ width: Math.max(3, Math.round(item.totalTokens / Math.max(1, stats.byModel[0].totalTokens) * 100)) + '%', background: colors[index % colors.length] }"></span></div></div></div></section>
        <section v-if="stats.bySource?.length" class="usage-section"><h3>{{ t('bySource') }}</h3><div class="usage-source-tags"><span v-for="item in stats.bySource" :key="item.key"><b>{{ sourceLabel(item.source) }}</b><em>{{ formatNumber(item.totalTokens) }}</em></span></div></section>
      </template>
    </div>
  </section>
</template>
