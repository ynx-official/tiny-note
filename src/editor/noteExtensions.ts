...
import type { JSONContent, MarkdownRendererHelpers, NodeViewRenderer, RenderContext } from '@tiptap/core'
...
function safeAlign(value: unknown): string {
  ...
  return /^(left|center|right|justify)$/.test(normalized) ? normalized : ''
}
...
function hasComplexCells(node: JSONContent): boolean {
...
function renderComplexTable(node: JSONContent): string {
...
const MarkdownSubscript = Subscript.extend({
  renderMarkdown: (node: JSONContent, helpers: MarkdownRendererHelpers) => `<sub>${helpers.renderChildren(node)}</sub>`
})

const MarkdownSuperscript = Superscript.extend({
  renderMarkdown: (node: JSONContent, helpers: MarkdownRendererHelpers) => `<sup>${helpers.renderChildren(node)}</sup>`
})
...
interface NoteExtensionOptions { lowlight?: ReturnType<typeof createLowlight>; codeBlockNodeView?: NodeViewRenderer; placeholder?: PlaceholderOptions['placeholder']; resizableTables?: boolean }
