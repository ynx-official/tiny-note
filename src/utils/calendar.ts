...
export interface CalendarDisplayItem { id: string; title: string; startDate: string; endDate: string; kind?: string; startTime?: string; endTime?: string; allDay?: boolean; completed?: boolean; color?: string; reminder?: { enabled?: boolean } | null; [key: string]: unknown }
const SOLAR_HOLIDAYS: Record<string, string> = { '01-01': '元旦', '02-14': '情人节', '03-08': '妇女节', '03-12': '植树节', '04-01': '愚人节', '05-01': '劳动节', '05-04': '青年节', '06-01': '儿童节', '07-01': '建党节', '08-01': '建军节', '09-10': '教师节', '10-01': '国庆节', '12-24': '平安夜', '12-25': '圣诞节' }
const LUNAR_HOLIDAYS: Record<string, string> = { '正月-初一': '春节', '正月-十五': '元宵节', '二月-初二': '龙抬头', '五月-初五': '端午节', '七月-初七': '七夕节', '七月-十五': '中元节', '八月-十五': '中秋节', '九月-初九': '重阳节', '腊月-初八': '腊八节', '腊月-廿三': '小年', '腊月-三十': '除夕' }
const LUNAR_MONTHS = ['', '正月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '冬月', '腊月']

function lunarMonthName(value: unknown) {
...
function lunarDayName(value: unknown) {
  const text = String(value || '')
...
export function monthCells(year: number, month: number, events: CalendarDisplayItem[] = []) {
...
export function yearMonths(year: number, events: CalendarDisplayItem[] = []) {
  return Array.from({ length: 12 }, (_, month) => ({ month, cells: monthCells(year, month, events).filter(cell => cell.isCurrentMonth) }))
}

export function holidayForDate(dateString: string) {
...
export function lunarLabelForDate(dateString: string) {
...
export function isInDateRange(date: string, start: string, end: string) {
  const low = start < end ? start : end
  const high = start < end ? end : start
  return Boolean(start && end && date >= low && date <= high)
}

export function localDateTime(date: string, time = '00:00') {
  return new Date(`${date}T${time || '00:00'}`)
}

export function todoCalendarItems(todos: Todo[]): CalendarDisplayItem[] {
  const colors: Record<string, string> = { high: '#C45A67', medium: '#B18B62', low: '#4E83A8', none: '#4B8F78' }
...
export function monthWeekRows(year: number, month: number, items: CalendarDisplayItem[] = [], maxVisibleLanes = 4) {
