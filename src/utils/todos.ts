...
export function filterTodos(items: Todo[], filter = 'inbox', now = new Date()) {
...
export function filterTodoViewItems(items: Todo[], filter = 'inbox', now = new Date()) {
...
export function groupTodos(items: Todo[], filter = 'inbox', now = new Date(), sortMode = 'due') {
  if (filter === 'completed') return [{ key: 'completed', items: sortTodos(items, sortMode) }]
  const today = localDateValue(now)
  const groups: Record<'overdue' | 'today' | 'later' | 'undated', Todo[]> = { overdue: [], today: [], later: [], undated: [] }
  ...
  const activeGroups: Array<{ key: string; items: Todo[] }> = (['overdue', 'today', 'later', 'undated'] as const)
...
export function defaultQuickDue(filter: string, now = new Date()) {
  if (filter === 'today') {
...
export function todoCounts(items: Todo[], now = new Date()) {
  return {
