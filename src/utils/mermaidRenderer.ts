...
interface MermaidRenderResult { svg: string }
...
function createConfig(theme: MermaidTheme) {
...
function decodeCodePoint(_match: string, value: string) {
  const codePoint = Number.parseInt(value, 16)
  return String.fromCodePoint(Math.min(codePoint, 0x10ffff))
}

function decodeEscapedResourceTokens(source: string) {
...
function externalResourceError() {
  const error = new Error('为保护隐私，图表不允许加载图片或外部资源。请改用普通节点和文字标签。') as Error & { code?: string }
  error.code = 'MERMAID_EXTERNAL_RESOURCE'
  return error
}

function validateMermaidSource(source: string) {
...
function hasUnsafeCssResource(cssText: string) {
...
function sanitizeSvg(svg: string): string {
  const rejectUnsafeInlineCss = (_node: Element, data: { attrName: string; attrValue: string; keepAttr: boolean }) => {
...
async function performRender(source: string, theme: MermaidTheme): Promise<MermaidRenderResult> {
 ...
 */
export function renderMermaidDiagram(source: string, { theme = 'light' }: { theme?: MermaidTheme } = {}): Promise<MermaidRenderResult> {
