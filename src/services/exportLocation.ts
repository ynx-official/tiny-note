...
interface ExportLocation { directory: string; remember: boolean }
interface ExportAppStore { settings: AppSettings; saveSettings(settings: AppSettings): Promise<AppSettings> }
export interface ExportSaveResult { path?: string; fileName?: string; browserDownload?: boolean; cancelled?: boolean }

const MAX_EXPORT_BYTES = 64 * 1024 * 1024
let resolveRequest: ((value: ExportLocation | null) => void) | null = null
...
function requestExportLocation() {
  ...
  return new Promise<ExportLocation | null>(resolve => { resolveRequest = resolve })
}

function finishRequest(value: ExportLocation | null) {
...
export async function chooseExportLocation() {
  ...
  try {
    ...
    exportLocationState.busy = false
    exportLocationState.error = errorMessage(error, '无法打开文件夹选择器')
  }
}

async function blobToBase64(blob: Blob) {
  if (blob.size > MAX_EXPORT_BYTES) {
    const error = new Error('导出文件超过 64 MB 限制') as Error & { code: string }
  ...
  const buffer = typeof blob.arrayBuffer === 'function'
    ...
    : await new Promise<ArrayBuffer>((resolve, reject) => {
        ...
        reader.addEventListener('load', () => reader.result instanceof ArrayBuffer ? resolve(reader.result) : reject(new Error('无法读取导出文件')), { once: true })
...
export async function saveExportBlob(blob: Blob, fileName: string, { appStore }: { appStore?: ExportAppStore } = {}): Promise<ExportSaveResult> {
  if (!isTauriRuntime()) {
