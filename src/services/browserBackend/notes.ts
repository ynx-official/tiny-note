...
function ensureTag(state: BrowserState, name: string, now: string): BrowserItem | null {
  ...
  if (!tag) { tag = item({ id: crypto.randomUUID(), name: normalized, version: 1, createdAt: now, updatedAt: now }); state.tags.push(tag) }
  return tag
}
...
export function handleNotesCommand(command: string, args: BrowserArgs, state: BrowserState, now: string, uncategorized: BrowserItem): BrowserHandlerResult | null {
  ...
  if (command === 'markdown_notebook_pick_folder') return { result: { selected: false, rootName: '', files: [], notebookPaths: [], ignoredDirectoryCount: 0, errors: [] } }
  ...
  if (command === 'note_set_pinned') { const note = state.notes.find(value => value.id === args.id); if (note) { Object.assign(note, { pinned: Boolean(args.pinned), updatedAt: now }); bumpVersion(note) } return { result: note || null } }
  ...
  if (command === 'note_create') { const note = item({ id: crypto.randomUUID(), notebookId: args.input.notebookId || uncategorized.id, knowledgeBaseId: args.input.knowledgeBaseId || null, title: args.input.title || '未命名笔记', contentHtml: args.input.contentHtml || '', contentText: args.input.contentText || '', contentMarkdown: args.input.contentMarkdown || '', pinned: Boolean(args.input.pinned), version: 1, deletedAt: null, createdAt: now, updatedAt: now }); state.notes.unshift(note); rebuildNoteLinks(state); return { result: note } }
  if (command === 'note_import_markdown') {
    const used = new Set(state.notebooks.filter(book => !book.parentId).map(book => String(book.name).toLowerCase()))
    const requestedRootName = String(args.rootName || 'Markdown 笔记本')
    const importFiles = Array.isArray(args.files) ? args.files as Array<Record<string, string>> : []
    let rootName = requestedRootName, suffix = 2
    while (used.has(rootName.toLowerCase())) rootName = `${requestedRootName} (${suffix++})`
    const root = item({ id: crypto.randomUUID(), parentId: null, name: rootName, description: '', version: 1, createdAt: now, updatedAt: now })
    state.notebooks.push(root)
    const directories = new Set<string>()
    for (const file of importFiles) { const parts = String(file.relativePath).split('/'); for (let depth = 1; depth < parts.length; depth++) directories.add(parts.slice(0, depth).join('/')) }
    const ids = new Map<string, string>([['', root.id]])
    for (const directory of [...directories].sort((a, b) => a.split('/').length - b.split('/').length || a.localeCompare(b))) { const parts = directory.split('/'); const parent = parts.slice(0, -1).join('/'); const book = item({ id: crypto.randomUUID(), parentId: ids.get(parent), name: parts.at(-1), description: '', version: 1, createdAt: now, updatedAt: now }); state.notebooks.push(book); ids.set(directory, book.id) }
    let firstNoteId = ''
    for (const file of importFiles) { const parts = String(file.relativePath).split('/'); const directory = parts.slice(0, -1).join('/'); const title = parts.at(-1)?.replace(/\.(?:md|markdown)$/i, '') || '导入笔记'; const note = item({ id: crypto.randomUUID(), notebookId: ids.get(directory), knowledgeBaseId: null, title, contentHtml: file.contentHtml || '', contentText: file.contentText || '', contentMarkdown: file.contentMarkdown || '', pinned: false, version: 1, deletedAt: null, createdAt: now, updatedAt: now }); firstNoteId ||= note.id; state.notes.unshift(note) }
    rebuildNoteLinks(state)
    return { result: { rootNotebookId: root.id, rootNotebookName: rootName, notebookCount: directories.size + 1, noteCount: importFiles.length, firstNoteId, renamed: rootName === requestedRootName ? {} : { [requestedRootName]: rootName } } }
  }
  if (command === 'note_open_external_markdown') { const existing = state.notes.find(note => note.externalPath === args.input.path); const note = existing || item({ id: crypto.randomUUID(), notebookId: uncategorized.id, knowledgeBaseId: null, pinned: false, version: 1, deletedAt: null, createdAt: now }); Object.assign(note, args.input, { notebookId: args.input.notebookId || uncategorized.id, externalPath: args.input.path, updatedAt: now }); if (existing) bumpVersion(note); else state.notes.unshift(note); rebuildNoteLinks(state); return { result: note } }
  if (command === 'note_update') { const note = state.notes.find(value => value.id === args.id); if (note) { Object.assign(note, args.input, { notebookId: args.input.notebookId || uncategorized.id, pinned: Boolean(args.input.pinned), updatedAt: now }); bumpVersion(note) } if (note) rebuildNoteLinks(state); return { result: note || null } }
  if (command === 'note_delete') { const note = state.notes.find(value => value.id === args.id); if (note) { note.deletedAt = now; bumpVersion(note); rebuildNoteLinks(state) } return { result: null } }
  if (command === 'note_copy') { const source = state.notes.find(note => note.id === args.id); if (!source) return { result: null }; const copy = item({ ...source, id: crypto.randomUUID(), title: `${source.title} 副本`, version: 1, createdAt: now, updatedAt: now }); state.notes.unshift(copy); state.noteTags.push(...state.noteTags.filter(link => link.noteId === source.id).map(link => ({ noteId: copy.id, tagId: link.tagId }))); rebuildNoteLinks(state); return { result: copy } }
  if (command === 'note_move') { const note = state.notes.find(value => value.id === args.id); if (note) { Object.assign(note, { notebookId: args.notebookId || uncategorized.id, updatedAt: now }); bumpVersion(note) } return { result: note || null } }
  if (command === 'note_move_to_knowledge_base') { const note = state.notes.find(value => value.id === args.id); if (!note) throw new Error('笔记不存在'); Object.assign(note, { knowledgeBaseId: args.knowledgeBaseId || null, updatedAt: now }); bumpVersion(note); return { result: note } }
  if (command === 'note_restore') { const note = state.notes.find(value => value.id === args.id); if (note) { note.deletedAt = null; bumpVersion(note); rebuildNoteLinks(state) } return { result: null } }
  ...
  if (command === 'notebook_create') { const id = crypto.randomUUID(); validateNotebookParent(state, id, args.parentId || null); const notebook = item({ id, parentId: args.parentId || null, name: args.name.trim(), description: args.description || '', version: 1, createdAt: now, updatedAt: now }); if (!notebook.name || notebook.name === '未分类') throw new Error('笔记本名称无效'); state.notebooks.push(notebook); return { result: notebook } }
  if (command === 'notebook_update') { const notebook = state.notebooks.find(value => value.id === args.id); const name = args.name.trim(); if (!notebook || notebook.name === '未分类') throw new Error('系统笔记本不能修改'); if (!name || name === '未分类') throw new Error('笔记本名称无效'); validateNotebookParent(state, notebook.id, args.parentId || null); Object.assign(notebook, { parentId: args.parentId || null, name, description: args.description || '', updatedAt: now }); bumpVersion(notebook); return { result: notebook } }
  if (command === 'notebook_move') { const notebook = state.notebooks.find(value => value.id === args.id); if (!notebook || notebook.name === '未分类') throw new Error('系统笔记本不能移动'); validateNotebookParent(state, notebook.id, args.parentId || null); Object.assign(notebook, { parentId: args.parentId || null, updatedAt: now }); bumpVersion(notebook); return { result: notebook } }
  ...
  if (command === 'tag_create') { const name = normalizeTags([args.name])[0]; if (!name) throw new Error('标签名称不能为空'); if (state.tags.some(tag => tag.name.toLocaleLowerCase() === name.toLocaleLowerCase())) throw new Error('标签已存在'); const tag = item({ id: crypto.randomUUID(), name, version: 1, createdAt: now, updatedAt: now }); state.tags.push(tag); return { result: tagDto(state, tag) } }
  if (command === 'tag_update') { const tag = state.tags.find(value => value.id === args.id); const name = normalizeTags([args.name])[0]; if (!tag || !name) throw new Error('标签不存在或名称无效'); if (state.tags.some(value => value.id !== tag.id && value.name.toLocaleLowerCase() === name.toLocaleLowerCase())) throw new Error('标签已存在'); Object.assign(tag, { name, updatedAt: now }); bumpVersion(tag); return { result: tagDto(state, tag) } }
