...
export function handlePlannerCommand(command: string, args: BrowserArgs, state: BrowserState, now: string): BrowserHandlerResult | null {
  ...
  if (command === 'todo_custom_list_create') { const list = item({ id: crypto.randomUUID(), ...todoListInput(args.input), createdAt: now, updatedAt: now }); state.todoLists.push(list); return { result: list } }
  if (command === 'todo_custom_list_update') { const list = state.todoLists.find(value => value.id === args.id); if (!list) throw new Error('清单不存在'); Object.assign(list, todoListInput(args.input), { updatedAt: now }); return { result: { ...list } } }
