...
export const useNotesStore = defineStore('notes', { state: () => ({ notes: [], deleted: [], notebooks: [], activeId: null, search: '', selectedNotebook: 'all', loading: false, saveTimer: null, saving: false }), getters: { active: s => s.notes.find(n => n.id === s.activeId) || null, visible: s => s.notes.filter(n => s.selectedNotebook === 'all' || n.notebookId === s.selectedNotebook) }, actions: {
  ...
  async create() { const n = await invoke('note_create', { input: { title: '未命名笔记', notebookId: this.selectedNotebook === 'all' ? null : this.selectedNotebook, knowledgeBaseId: null, contentHtml: '<p></p>', contentText: '', contentMarkdown: '' } }); this.notes.unshift(n); this.activeId = n.id; return n },
  async createFromContent({ title = '未命名笔记', contentHtml = '<p></p>', contentText = '', contentMarkdown = '', notebookId, knowledgeBaseId = null } = {}) { const uncategorizedId = this.notebooks.find(book => book.name === '未分类')?.id || null; const n = await invoke('note_create', { input: { title: title.trim() || '未命名笔记', notebookId: notebookId === undefined ? uncategorizedId : notebookId, knowledgeBaseId, contentHtml, contentText, contentMarkdown } }); this.notes.unshift(n); this.activeId = n.id; return n },
  async importText(file) { const text = await file.text(); const title = file.name.replace(/\.[^.]+$/, '') || '导入笔记'; const extension = file.name.split('.').pop()?.toLowerCase(); const escaped = text.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('\n', '<br>'); const isHtmlNote = extension === 'note' && /<\w[\s\S]*>/.test(text); const html = extension === 'md' || extension === 'markdown' ? sanitizeEditorHtml(marked.parse(text)) : isHtmlNote ? sanitizeEditorHtml(text) : `<p>${escaped}</p>`; const contentMarkdown = isHtmlNote ? '' : text; const contentText = extension === 'md' || extension === 'markdown' || isHtmlNote ? textFromEditorHtml(html) : text; const n = await invoke('note_create', { input: { title, notebookId: this.selectedNotebook === 'all' ? null : this.selectedNotebook, knowledgeBaseId: null, contentHtml: html, contentText, contentMarkdown } }); this.notes.unshift(n); this.activeId = n.id; return n },
  async save(note) { this.saving = true; try { const updated = await invoke('note_update', { id: note.id, input: { title: note.title, notebookId: note.notebookId, knowledgeBaseId: note.knowledgeBaseId || null, contentHtml: note.contentHtml, contentText: note.contentText, contentMarkdown: note.contentMarkdown || '' } }); if (updated) Object.assign(note, updated) } finally { this.saving = false } },
  ...
  async rename(id, title) {
    ...
    const updated = await invoke('note_update', { id, input: { title: title.trim(), notebookId: note.notebookId, knowledgeBaseId: note.knowledgeBaseId || null, contentHtml: note.contentHtml, contentText: note.contentText, contentMarkdown: note.contentMarkdown || '' } })
  ...
  async moveToKnowledge(id, knowledgeBaseId) { const updated = await invoke('note_move_to_knowledge_base', { id, knowledgeBaseId: knowledgeBaseId || null }); const note = [...this.notes, ...this.deleted].find(item => item.id === id); if (note && updated) Object.assign(note, updated); return updated || note },
