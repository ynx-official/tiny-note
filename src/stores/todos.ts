...
export const useTodosStore = defineStore('todos', {
  ...
  actions: {
    ...
    async updateList(id: string, input: Partial<TodoList>) { const item = await invoke('todo_custom_list_update', { id, input }); const index = this.lists.findIndex(value => value.id === id); if (index >= 0) this.lists[index] = item; return item },
    ...
    async update(id: string, input: TodoInput) { const item = await invoke('todo_update', { id, input }); this.upsert(item); return item },
    async setCompleted(id: string, completed: boolean) { const item = await invoke('todo_set_completed', { id, completed }); this.upsert(item); return item },
