...
<template><AppShell :active="active"><router-view /></AppShell><div class="task-toast-region" aria-live="polite"><button v-for="notice in tasks.notices" :key="notice.id" type="button" class="task-toast" @click="tasks.dismissNotice(notice.id)">{{ notice.message }}</button></div></template>
