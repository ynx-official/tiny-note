...
合并 `dev` 的外部来源管理时，`external_markdown_pick_files`、`external_markdown_pick_folder` 和 `external_markdown_remove` 保留为桌面平台命令。选择器只授权用户选中的 Markdown 文件；目录扫描不跟随符号链接。单条移除只更新本机 JSON 授权映射，不删除源文件，也不重新引入旧 SQLite 业务层。
