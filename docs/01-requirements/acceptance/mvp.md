...
- Given Markdown 包含合法的 `mermaid` 流程图或 `swimlane-beta` 泳道图，When 在即时编辑或 Markdown 预览查看，Then 显示图形而不是普通代码文本，围栏源码可无损继续编辑和导出。
- Given Mermaid 图表较宽，When 首次渲染、源码变化或适宽状态下容器尺寸变化，Then 行内与全屏视图默认重新适合当前宽度并给出完整概览；When 用户主动缩放，Then 保留手动缩放状态，100% 保持 SVG 原始比例、图表保持矢量清晰且滚动只发生在图表舞台内；全屏可在 10%–250% 间查看，按住鼠标左键可拖动画布，滚轮围绕鼠标指向的位置缩放；Esc 可关闭全屏并恢复焦点。
...
- Given Markdown 草稿刚刚发生变化且尚未经过 150ms 解析防抖，When 用户立即打印或导出 HTML/PDF，Then 输出包含最新草稿、独立标题和安全正文，且不包含应用外壳、工具栏、AI 面板或瞬时 Mermaid SVG。
