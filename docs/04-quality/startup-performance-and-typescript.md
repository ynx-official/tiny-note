...
最后更新：2026-09-14。2026-08-27 测量值保留为历史证据，当前加载交互及验证范围见[启动与笔记加载连续性](interaction-continuity.md)。
...
`index.html` 在 head 中通过 CSP 哈希授权的脚本解析设备主题，再加载不足 2KB 的 `public/boot.css` 静态外壳。Tauri 原生窗口仍使用配置中的初始背景色。`src/main.ts` 不再等待启动完成动画，模块准备好后挂载；首页内容保持挂载，设置与模型异步校准。首页为唯一 eager route，其余页面通过动态导入切分；功能 CSS、编辑器、Mermaid 与 PDF 导出不进入首页启动闭包。当前业务数据使用远程服务，不执行旧 SQLite 迁移；托盘继续使用独立的 `tray` 入口。
