...
`image_asset_read` 调用 `GET /image-assets/{id}`，直接使用响应中的 `dataUri`。服务端先校验图片文档归属和私有对象前缀，再读取 COS/S3，按已有 20 MB 图片上限验证声明大小和实际字节数，返回图片 Data URI；响应使用 `Cache-Control: private, no-store`。图片列表不内嵌内容。

客户端不再 `fetch(downloadUrl)`，因此预览、保存、插入笔记和复用图片不依赖存储桶 CORS。后端保留短期 `downloadUrl` 字段兼容旧接口消费者；新客户端必须搭配支持 `dataUri` 的后端部署。该修复不改变存储桶访问策略、用户权限或数据库结构。
