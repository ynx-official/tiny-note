...
`external_markdown_sources` 以 `note_id` 一对一关联从系统打开的 Markdown，保存规范化绝对路径和上次同步内容的 SHA-256；路径唯一，避免同一源文件产生多个临时记录。存在该关联的 `notes` 行只服务于当前设备上的打开、编辑和源文件同步，不属于用户笔记集合，所有笔记列表、标签、双向链接、检索索引和工作区备份必须排除。用户导入时新建无此关联的普通 `notes` 副本，不能把外部记录原地转为笔记。保存外部记录时先比较当前磁盘哈希，匹配后以同目录临时文件原子替换并更新哈希；冲突、文件丢失、非 UTF-8 或超过 10 MB 时不得更新持久状态或覆盖磁盘。
