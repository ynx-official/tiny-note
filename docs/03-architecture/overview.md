...
文章 HTML/PDF 在前端从二次清洗后的语义快照生成，不新增任意文件系统权限或 Rust 导出命令；PDF 渲染依赖按需加载。系统打印使用 WebView 的 `window.print()`，macOS 由 Tauri 转发为异步原生命令，因此主窗口 capability 仅额外放行 `core:webview:allow-print`，调用方等待命令完成并处理拒绝。
